#!/bin/bash
# Full-run launcher: 200 records × 10 configs for MedQA.
#
# Run on the GPU VM inside a tmux session:
#   tmux new -s full_medqa
#   source ~/final_project/.venv/bin/activate
#   cd ~/final_project
#   bash uncertainty_benchmark/scripts/run_full_medqa.sh 2>&1 | tee ~/full_medqa.log
#
# Order: Gemini (API, no GPU) → Qwen → Gemma → DeepSeek
# Estimated wall-clock: ~33 hours
...
echo "======================================================"
echo "  Uncertainty Benchmark — Full Runs: MedQA (N=${N})"
echo "  $(date)"
echo "======================================================"
...
for method in single flex; do
    run_config "medqa" "gemini-2.5-flash" "$method"
done

echo ""
echo "▶▶▶  GEMINI-3.1-PRO-PREVIEW  ◀◀◀"
for method in single flex; do
    run_config "medqa" "gemini-3.1-pro-preview" "$method"
done
...
echo "▶▶▶  QWEN3-4B  ◀◀◀"
for method in single flex; do
    run_config "medqa" "qwen3-4b" "$method"
done
...
echo "▶▶▶  GEMMA-3-12B-IT  ◀◀◀"
for method in single flex; do
    run_config "medqa" "gemma-3-12b-it" "$method"
done
...
for method in single flex; do
    run_config "medqa" "deepseek-r1-distill-70b" "$method"
done

# ── Summary ───────────────────────────────────────────────────────────────────
echo ""
echo "======================================================"
echo "  MEDQA FULL-RUN SUMMARY"
echo "  $(date)"
echo "======================================================"
