#!/bin/bash
# Full-run launcher: 200 records × 10 configs for MS-Dialog.
#
# Run on the GPU VM inside a tmux session:
#   tmux new -s full_msdialog
#   source ~/final_project/.venv/bin/activate
#   cd ~/final_project
#   bash uncertainty_benchmark/scripts/run_full_msdialog.sh 2>&1 | tee ~/full_msdialog.log
#
# Order: Gemini (API, no GPU) → Qwen → Gemma → DeepSeek
# Estimated wall-clock: ~10 hours
...
echo "======================================================"
echo "  Uncertainty Benchmark — Full Runs: MS-Dialog (N=${N})"
echo "  $(date)"
echo "======================================================"
...
for method in single flex; do
    run_config "msdialog" "gemini-2.5-flash" "$method"
done

echo ""
echo "▶▶▶  GEMINI-3.1-PRO-PREVIEW  ◀◀◀"
for method in single flex; do
    run_config "msdialog" "gemini-3.1-pro-preview" "$method"
done
...
echo "▶▶▶  QWEN3-4B  ◀◀◀"
for method in single flex; do
    run_config "msdialog" "qwen3-4b" "$method"
done
...
echo "▶▶▶  GEMMA-3-12B-IT  ◀◀◀"
for method in single flex; do
    run_config "msdialog" "gemma-3-12b-it" "$method"
done
...
for method in single flex; do
    run_config "msdialog" "deepseek-r1-distill-70b" "$method"
done

# ── Summary ───────────────────────────────────────────────────────────────────
echo ""
echo "======================================================"
echo "  MS-DIALOG FULL-RUN SUMMARY"
echo "  $(date)"
echo "======================================================"
...
if [ ${#FAILURES[@]} -gt 0 ]; then
    ...
    echo ""
    echo "  All MS-Dialog runs PASSED."
    exit 0
fi
